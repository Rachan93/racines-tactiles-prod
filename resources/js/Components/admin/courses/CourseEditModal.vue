...
const isDraggingCover = computed(() => coverDragDepth.value > 0);
const coverPreviewUrl = computed(
    () => selectedCoverUrl.value || props.course?.cover_image_url || null,
);
const resetCoverSelection = () => {
    if (selectedCoverUrl.value) URL.revokeObjectURL(selectedCoverUrl.value);
...
const setCoverFile = (file) => {
    ...
    if (!["image/jpeg", "image/png", "image/webp"].includes(file.type)) {
        coverImageError.value = "Formats acceptés : JPG, PNG et WebP.";
        return;
    }
    if (file.size > 5 * 1024 * 1024) {
        coverImageError.value = "L’image ne doit pas dépasser 5 Mo.";
        return;
    }
    if (selectedCoverUrl.value) URL.revokeObjectURL(selectedCoverUrl.value);
...
const handleCoverDrop = (event) => {
    ...
    if (files.length > 1) {
        coverImageError.value = "Choisissez une seule image de couverture.";
        return;
    }
    setCoverFile(files[0]);
};
onBeforeUnmount(resetCoverSelection);
...
const isAllOverriddenSelected = computed({
    ...
    set: (isChecked) => {
        if (isChecked) {
            selectedLessonIds.value = futureOverriddenLessons.value.map((l) =>
                Number(l.id),
            );
        } else {
...
const populateForm = (courseData) => {
    ...
    form.default_instructor_id = courseData.default_instructor_id
        ...
        : courseData.instructor?.id
          ? String(courseData.instructor.id)
          : "";
    ...
    form.default_spots_max_handbuilding =
        courseData.default_spots_max_handbuilding ?? 2;
...
const submit = () => {
    if (!props.course || form.processing || coverImageError.value) return;
    form.submit({
        preserveScroll: true,
        forceFormData: true,
        onSuccess: () => {
    ...
    <Dialog :open="open" @update:open="(val) => emit('update:open', val)">
        ...
        >
            ...
            <DialogHeader class="p-6 pb-4 border-b shrink-0 bg-muted/20 pr-12">
                <div class="space-y-1">
                    <div class="flex items-center gap-2 flex-wrap">
                        ...
                        <Badge
                            variant="outline"
                            :class="getTypeBadgeClass(course?.type?.name)"
                    ...
                    <DialogDescription class="text-xs">
                        Modifiez les valeurs par défaut et le contenu descriptif
                        public.
                    </DialogDescription>
                </div>
            </DialogHeader>
            ...
            >
                ...
                <div class="space-y-3">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        <div class="space-y-1.5">
                                ...
                                @update:model-value="
                                    (val) => {
                                        form.default_instructor_id =
                                            String(val);
                                        form.validate('default_instructor_id');
                                    }
                                "
                            >
                                <SelectTrigger
                                    id="default_instructor"
                                    class="bg-background text-xs"
                                    :class="{
                                        'border-destructive':
                                            form.errors.default_instructor_id,
                                    }"
                                >
                                    <SelectValue
                            ...
                            <p
                                v-if="form.errors.default_instructor_id"
                                class="text-[10px] text-destructive mt-1"
                            >
                                {{ form.errors.default_instructor_id }}
                            </p>
                        ...
                        <div class="space-y-1.5">
                            <Label
                                ...
                                >Formule associée</Label
                            >
                            <div
                                class="h-9 px-3 rounded-md border bg-muted/40 text-xs text-foreground flex items-center justify-between"
                            >
                                <span>{{ course?.type?.name }}</span>
                                <Badge
                                    variant="outline"
                                    class="text-[10px] bg-background"
                                    >Fixe</Badge
                                >
                            </div>
                        </div>
                ...
                <div
                    v-if="isStage"
                    class="space-y-4 p-4 rounded-xl border bg-muted/20 border-amber-300/40"
                >
                                    ...
                                    <Tooltip>
                                        ...
                                        <TooltipContent side="top">
                                            <p class="text-xs max-w-xs">
                                                Si activé, ce stage apparaîtra
                                                en tête d'affiche sur la page
                                                publique des stages.
                                            </p>
                                        </TooltipContent>
                                    </Tooltip>
                    ...
                    <div class="space-y-1.5">
                        ...
                        >
                            <SelectTrigger
                                ...
                                class="bg-background text-xs"
                                :class="{
                                    'border-destructive': form.errors.sub_type,
                                }"
                            >
                                <SelectValue
                    ...
                    <Tabs default-value="fr" class="w-full pt-1">
                        ...
                        <TabsContent value="fr" class="space-y-3 mt-0">
                            ...
                            <div class="space-y-1.5">
                                ...
                                ></textarea>
                                <p
                                    v-if="form.errors.practical_info"
                                    class="text-[10px] text-destructive mt-1"
                                >
                                    {{ form.errors.practical_info }}
                                </p>
                        ...
                        <TabsContent value="en" class="space-y-3 mt-0">
                            ...
                            <div class="space-y-1.5">
                                ...
                                ></textarea>
                                <p
                                    v-if="form.errors.description_en"
                                    class="text-[10px] text-destructive mt-1"
                                >
                                    {{ form.errors.description_en }}
                                </p>
                            ...
                            <div class="space-y-1.5">
                                ...
                                <textarea
                                    ...
                                    :class="{
                                        'border-destructive':
                                            form.errors.practical_info_en,
                                    }"
                                    @change="form.validate('practical_info_en')"
                                ></textarea>
                                <p
                                    v-if="form.errors.practical_info_en"
                                    class="text-[10px] text-destructive mt-1"
                                >
                                    {{ form.errors.practical_info_en }}
                                </p>
                    ...
                    <!-- Couverture commune aux deux langues. -->
                    <section
                        class="space-y-3 rounded-lg border bg-background p-4"
                        aria-labelledby="stage-image-title"
                    >
                        <div class="space-y-1">
                            <h3 id="stage-image-title" class="text-sm font-semibold">
                                Image de couverture
                                <span class="font-normal text-muted-foreground">(facultative)</span>
                            </h3>
                            <p id="stage-image-help" class="text-xs leading-relaxed text-muted-foreground">
                                Une image pour illustrer la carte du stage, commune au français et à l’anglais.
                                JPG, PNG ou WebP · 5 Mo maximum. Privilégiez une photo avec le sujet au centre.
                            </p>
                        </div>
                        <div
                            class="rounded-lg border-2 border-dashed p-3 transition-colors"
                            :class="isDraggingCover ? 'border-primary bg-primary/5' : 'border-muted-foreground/25'"
                            @dragenter.prevent="!form.processing && coverDragDepth++"
                            @dragover.prevent
                            @dragleave.prevent="coverDragDepth = Math.max(0, coverDragDepth - 1)"
                            @drop.prevent="handleCoverDrop"
                        >
                            <div v-if="coverPreviewUrl" class="overflow-hidden rounded-md border bg-muted/30">
                                <img
                                    :key="coverPreviewUrl"
                                    :src="coverPreviewUrl"
                            ...
                            <div
                                ...
                                :class="coverPreviewUrl ? 'pt-3' : 'min-h-40'"
                            >
                                <p v-if="!coverPreviewUrl" class="text-xs text-muted-foreground">
                                    Glissez une image ici ou choisissez un fichier.
                                </p>
                                <div class="flex flex-wrap items-center justify-center gap-2">
                                    ...
                                    >
                                        {{ coverPreviewUrl ? 'Remplacer l’image' : 'Choisir une image' }}
                                    </Button>
                                    <Button
                                        v-if="form.cover_image || coverImageError"
                                        type="button" variant="ghost" size="sm"
                                        :disabled="form.processing"
                                        @click="resetCoverSelection"
                                    >
                            ...
                            <input
                                ...
                                :aria-invalid="Boolean(coverImageError || form.errors.cover_image)"
                                :disabled="form.processing"
                                @change="selectCoverImage"
                            />
                        </div>
                        <p v-if="coverPreviewUrl" class="text-center text-xs text-muted-foreground">
                            Cet aperçu ne représente pas l’affichage final :
                            le cadrage et les dimensions pourront varier sur la carte du stage.
                        </p>
                        <p id="stage-image-error" role="alert" class="text-xs text-destructive">
                            {{ coverImageError || form.errors.cover_image }}
                        </p>
                        <p class="text-xs text-muted-foreground">
                            La nouvelle image sera envoyée à l’enregistrement.
                            Sans nouveau fichier, la couverture actuelle est conservée.
                        </p>
                        <div
                            v-if="form.processing && form.cover_image && form.progress"
                            class="space-y-1" role="status" aria-live="polite"
                        >
                            <progress
                                class="h-2 w-full"
                                :value="form.progress.percentage || 0"
                                max="100" aria-label="Envoi de l’image"
                            ></progress>
                            <p class="text-xs text-muted-foreground">
                                Envoi : {{ form.progress.percentage || 0 }} %
                            </p>
                        </div>
                    </section>
                </div>
                ...
                <div class="space-y-3">
                    <h4
                        class="text-xs font-bold text-muted-foreground uppercase tracking-wider flex items-center gap-1.5"
                    >
                        <Clock class="h-3.5 w-3.5" /> Horaires & Tarif par
                        défaut
                    </h4>
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <div class="space-y-1.5">
                            <Label
                                for="course_start_time"
                                class="text-xs font-semibold"
                            ...
                            />
                            <p
                                v-if="form.errors.default_start_time"
                                class="text-[10px] text-destructive mt-1"
                            >
                                {{ form.errors.default_start_time }}
                            </p>
                ...
                <div class="space-y-3">
                    <h4
                        class="text-xs font-bold text-muted-foreground uppercase tracking-wider flex items-center gap-1.5"
                    >
                        <Layers class="h-3.5 w-3.5" /> Capacités d'accueil par
                        défaut
                    </h4>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <!-- Tours (Shell) -->
                        <div
                            class="p-3 rounded-lg bg-background border space-y-1.5"
                        >
                            <Label
                                for="wheel_default_spots"
                            ...
                            <Input
                                ...
                                :class="{
                                    'border-destructive':
                                        form.errors.default_spots_max_wheel,
                                }"
                                @change="
                                    form.validate('default_spots_max_wheel')
                                "
                            />
                            <p
                                v-if="form.errors.default_spots_max_wheel"
                                class="text-[10px] text-destructive mt-1"
                            >
                                {{ form.errors.default_spots_max_wheel }}
                            </p>
                        ...
                        >
                            ...
                            <p
                                v-if="
                                    form.errors.default_spots_max_handbuilding
                                "
                                class="text-[10px] text-destructive mt-1"
                            >
                                {{ form.errors.default_spots_max_handbuilding }}
                ...
                <!-- ========================================================= -->
                <div
                    class="p-3 rounded-lg bg-muted/40 border text-xs flex items-center justify-between text-muted-foreground"
                >
                ...
                >
                    <div class="flex items-start gap-2.5">
                        ...
                        <div class="text-xs space-y-1">
                            <p
                                class="font-bold text-amber-950 dark:text-amber-200"
                            >
                                {{
                                    pluralize(
                                        futureOverriddenLessons.length,
                                        "séance future personnalisée",
                                    )
                                }}
                            </p>
                            <p
                            ...
                            >
                                Cochez les séances que vous souhaitez réaligner
                                sur ces nouvelles valeurs par défaut.
                            </p>
                        </div>
                    </div>
                    <!-- Master Checkbox (Tout sélectionner / Tout désélectionner) -->
                    <div
                        class="flex items-center gap-2.5 p-2 rounded-lg bg-amber-100/60 dark:bg-amber-900/30 border border-amber-300/60 dark:border-amber-800/50"
                    >
                        <Checkbox
                            ...
                            @update:checked="
                                (val) => {
                                    isAllOverriddenSelected = Boolean(val);
                                }
                            "
                            @update:model-value="
                                (val) => {
                    ...
                    <ScrollArea
                        class="h-48 w-full rounded-lg border bg-background p-2 overscroll-contain"
                    >
                        <div class="space-y-1.5 pr-2">
                            <div
                                ...
                                :class="
                                    selectedLessonIds.includes(Number(l.id))
                            ...
                            >
                                <div class="flex items-center gap-2.5 min-w-0">
                                    ...
                                    >
                                        ...
                                        <div
                                            class="text-[11px] text-muted-foreground flex items-center gap-2 flex-wrap"
                                        >
                                            <span
                                            ...
                                            >
                                                ...
                                                <Badge
                                                    ...
                                                    "
                                                    variant="outline"
                                                    class="text-[9px] py-0 px-1 text-amber-600 border-amber-300"
                                                >
                                                    Remplaçant
                                                </Badge>
                                            </span>
                                ...
                                <div
                                    class="flex items-center gap-2 text-[11px] text-muted-foreground shrink-0 pl-2"
                                >
                                    <span
                                        class="inline-flex items-center gap-0.5 text-sky-700 dark:text-sky-300 font-medium"
                                    >
                                        <Shell class="h-3 w-3" />
                                        {{ l.spots.wheel_max }}
                                    </span>
                                    <span
                                        class="inline-flex items-center gap-0.5 text-orange-700 dark:text-orange-300 font-medium"
                                    >
                                        <Hand class="h-3 w-3" />
                                        {{ l.spots.handbuilding_max }}
                                    </span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="flex items-center gap-1">
                        ...
                        <TooltipProvider>
                            <Tooltip>
                                ...
                                <TooltipContent side="top">
                                    <p class="text-xs max-w-xs">
                                        Si activé, ce cours est visible dans le
                                        calendrier et ouvert aux réservations.
                                    </p>
                                </TooltipContent>
                            </Tooltip>
                        </TooltipProvider>
